...
- [ ] **全局搜索**：新增 `/api/search?q=` 递归扫描，顶栏搜索框全库生效（现在只能过滤当前目录）。
- [ ] **剧集连播**：文件名自然排序（"第2集"排在"第10集"前）；播完自动播同目录下一个视频；
  播放页加"下一集"按钮。
- [ ] **多字幕挂载**：同名/同前缀的多个 `.srt/.vtt`（如 `.chs/.eng`）全部挂成 track，
  用浏览器原生菜单切换（现在只挂第一个匹配的）。
- [ ] **记住倍速和音量**：localStorage 持久化，刷新后不重置。

## 阶段 2：体验补强

- [ ] **进度服务端化 + 首页"继续观看"**：新增 `/api/progress`，进度落服务端 JSON 文件
  （保持零依赖），跨设备续看；首页顶部加"继续观看"一行。
- [ ] **不支持格式的显式处理**：列表页给 avi/wmv/rmvb 等打"不支持"角标；播放失败时
  给出转码命令提示。进阶：启动时检测系统 ffmpeg，有则开实时转码端点（可选能力，不破坏零依赖）。
- [ ] **浏览页状态保持**：从播放页返回时不再重新 fetch、滚动归零；按路径缓存 listing + 恢复滚动位置。

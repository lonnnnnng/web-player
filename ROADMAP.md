...
- [x] 进度存储加固：按比例存储在文件替换后会错位，改为存绝对秒数并校验 duration；
  timeupdate 每 3 秒节流保存，在 `pagehide`/`visibilitychange` 补一次 flush。
- [x] 首次访问保留默认音量；静音开关按实际声音状态切换，零音量可一次恢复。
- [x] 进度同步等待落盘确认，持久化重试队列；毫秒版本排序和删除标记防止乱序回退。
- [x] 存储禁用/配额不足时会话兜底并继续同步；缩略图设置容量上限。
- [x] 浏览目录与播放队列隔离，切页取消迟到目录和延迟搜索的回写。
- [x] 服务端读写失败返回明确错误，不覆盖损坏记录；停止脚本验证进程身份。
...
- [x] **不支持格式的显式处理**：列表页给 avi/wmv/rmvb 等打"不支持"角标；播放失败时
  给出错误与重试入口，转码说明保留在 README。（ffmpeg 实时转码端点移入远期）
...
- [x] 媒体库、最近播放、文件夹导航，浅色/深色主题及本地 Lucide 图标
- [x] 全库分页、类型筛选、三种排序、网格/列表偏好
- [x] 音频触屏进度条、播放/暂停、前后跳转及同目录队列
- [x] 加载骨架、空列表、错误重试及进度同步状态
- [x] 搜索播放后返回保留条件，等待续播选择时不覆盖已有记录
- [ ] Android / iPhone 真机验证系统音量、锁屏与后台行为

## 已知不做的

- 上传/管理功能：违背"播放已下载资源"的定位
- 运行服务时强制 npm 构建：已生成的图标随代码提供；应用运行时保持 Python 标准库 + 浏览器原生 API

...
一个零依赖（仅 Python 标准库）的本地视频网页播放器，部署到 Linux 服务器后，
用浏览器就能播放服务器上已下载好的视频资源。

## 功能

- 📂 **自定义视频目录**：命令行参数 / 环境变量 / config.json 三种方式指定，支持多级子目录浏览
- 🎬 **在线流式播放**：支持 HTTP Range 请求，可随意拖动进度条、边下边播
- ⏯️ **播放进度记忆**：每个视频记住看到的位置，下次打开提示"继续播放"
- 💬 **自动加载字幕**：视频同名目录下有 `.srt` / `.vtt` 字幕文件时自动挂载（SRT 自动转 VTT）
- ⏩ **倍速播放**：0.5x ~ 3x
- ⌨️ **快捷键**：`空格` 播放/暂停，`←/→` 快退/快进 10 秒，`↑/↓` 音量，`F` 全屏，`Esc` 返回
- 🔍 **目录内搜索**、文件大小/修改时间显示、视频时长懒加载显示
- 🔒 **可选密码保护**（HTTP Basic）
...
无论哪种方式，启动后浏览器访问 `http://服务器IP:8080` 即可。
...
|------|----------|-------------|------|
| `--dir` | `VIDEO_DIR` | `video_dir` | 视频资源目录（**必填其一**，默认 `./videos`） |
...
- 首页列出视频根目录，点击文件夹进入子目录，点击视频卡片开始播放
- 支持的格式：浏览器能直接解码的格式（**mp4 / webm / mov / m4v** 最稳；
  **mkv** 在 Chrome/Edge 通常可以；**avi / wmv / flv / rmvb** 等老格式浏览器不支持，
  建议转码：`ffmpeg -i 1.avi -c:v libx264 -c:a aac 1.mp4`）
- 字幕：把 `xxx.srt` 放在视频旁边并同名（如 `电影.mp4` + `电影.srt`），播放时自动加载
